...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setServerError(err.response?.data?.message || "Wrong email or password. Try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white px-4 py-10"
      ...
      <div className="w-full max-w-sm sm:border sm:border-[#dadce0] sm:rounded-lg sm:shadow-sm px-4 sm:px-10 py-8 sm:py-12">
        ...
        <p className="text-center text-base text-[#202124] mb-8">
          to continue to YouTube
        </p>

        {/* Server error banner */}
        {serverError && (
          <div className="flex items-start gap-2 bg-[#fce8e6] border border-[#f5c6c2] rounded px-3 py-2.5 mb-6">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              autoComplete="email"
              className={`w-full h-[52px] px-3 border rounded text-[15px] text-[#202124] bg-white placeholder-[#9aa0a6]
                outline-none transition-colors
                focus:border-[#1a73e8] focus:ring-1 focus:ring-[#1a73e8]
                ${errors.email ? "border-[#cc0000] ring-1 ring-[#cc0000]" : "border-[#dadce0]"}`}
            />
            {errors.email && (
              <span className="text-[#cc0000] text-xs pl-1">{errors.email}</span>
            )}
          </div>

          {/* Password field */}
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-[#3c4043]" htmlFor="password">
              Password
            </label>
            <div className="relative">
              <input
                ...
                className={`w-full h-[52px] px-3 pr-11 border rounded text-[15px] text-[#202124] bg-white placeholder-[#9aa0a6]
                  outline-none transition-colors
                  focus:border-[#1a73e8] focus:ring-1 focus:ring-[#1a73e8]
          ...
          <div className="flex items-center justify-between mt-2 gap-3 flex-wrap">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center justify-center gap-2 px-6 h-10 bg-[#1a73e8] hover:bg-[#1765cc] disabled:bg-[#7fb3f5] text-white text-sm font-medium rounded transition-colors cursor-pointer disabled:cursor-not-allowed min-w-[90px]"
            >
              {loading ? (
                <>
